...
__global__ void sum_kernel(float const* const input,
    ...
    for (int numActiveThreads = blockDim.x; numActiveThreads >= 1; numActiveThreads /= 2) {
        if (x < numActiveThreads) {
            buffer[x] = buffer[x] + (x + numActiveThreads < bufferLength ? buffer[x + numActiveThreads] : 0);
        }
        __syncthreads();
    }
    if (x == 0) {
