...
void solve(const float* input, float* output, int rows, int cols) {
    ...
    int sharedMemory = COARSENING_FACTOR * BLOCK_SIZE * (BLOCK_SIZE+1) * sizeof(float);
    cudaFuncSetAttribute(matrix_transpose_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, sharedMemory);
    matrix_transpose_kernel<<<blocksPerGrid, threadsPerBlock, sharedMemory>>>(input, output, rows, cols);
}
